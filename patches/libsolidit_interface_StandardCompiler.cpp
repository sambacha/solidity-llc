...
boost::variant<StandardCompiler::InputsAndSettings, Json::Value> StandardCompiler::parseInput(Json::Value const& _input)
{
 ...
 //   else if (_input["language"] == "Solidity")  
    //       return compileSolidity(_input);
 //       else
   //         return formatFatalError("JSONError", "Unknown language: " + _input["language"].asString());
    ...
    // if (auto result = checkSourcesKeys(sources))
    // TODO: check if sources is an object
